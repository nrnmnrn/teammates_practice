...
> 完成條件：完整三個 tabs、真實排程與指標、Mock adaptation，且已接通隊友後端。
...
採 Windows、Conda `scheduler-ui` 與 Python 3.11.16。使用 uv 安裝套件，明確指定 Conda 環境的 Python，不另外建立專案 `.venv`。
...
在已可使用 Conda、uv 的 PowerShell／Anaconda PowerShell Prompt 中：
...
若環境已存在，跳過建立步驟。專案指令不得寫死原開發者的帳號或絕對路徑。
...
| 30–45 秒 | 提出 Mock Hybrid 候選，說明預寫規則與未經 evaluator 驗證，再按接受。 |
