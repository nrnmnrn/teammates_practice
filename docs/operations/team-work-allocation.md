...
隊員承接已核准且內容完整的子 PRD 時，不必重做完整訪談。子 PRD 本身就是已寫清楚的 spec。先執行一次 `$to-tickets`，再按順序讓每張 Ticket 各執行一次 `$implement`。每張 Ticket 確認完成後，執行 `$task-closeout`，留下狀態、證據與下一步。
...
4. Ticket 已真正完成時，執行 `$task-closeout`。它核對完成狀態、改動範圍與下一張優先工作；`$implement` 已 commit 時，closeout 不需要再產生第二個 commit。
